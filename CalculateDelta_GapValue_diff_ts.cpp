...
int main(){
  ...
  Calculate_Delta( S_0,  Sigma,  Barrier,  Bonus_Level,  mu,  r_f,  epsilon,  maturity,  5, 250 );
...
void Calculate_Delta(const double S_0, const double Sigma, const double Barrier, const double Bonus_Level, const double mu, const double r_f, const double epsilon, const double maturity, const int N_paths, const int N_timeSteps){
	...
	std::ofstream file_output_Delta_if;
	file_output_Delta_if.open("Delta_if_" + std::to_string(path) + ".csv");
	file_output_Delta_if << "time" << "," << "S_tmp_mu" << "," << "D_tmp" << "," << "D_tmp_pls_epsilon" << "," << "D_tmp_min_epsilon" << "," << "delta" << "," << "gamma" << 
	"," << "gamma1" << "," << "kosten_aktien_Kauf_Verkauf" << "," << "zinskostenAktien" << "," << "RiskFreeAsset_vorUmschichtung" << "," 
	<< "RiskFreeAsset_NachUmschichtung" << "," << "Gap_value_vorUmsch" << "," << "Gap_valueNachUmsch" << "," << "Sum_kosten_aktien_Kauf_Verkauf" << "," << "Sum_zinskostenAktien" << "\n";
	...
	double gamma = 0.0;
	double gamma1 = 0.0;
	double gamma1_if = 0.0;


	double RiskFreeAsset_beforeRebalancing = 0.0;
	double RiskFreeAsset_beforeRebalancing_if = 0.0;
	double RiskFreeAsset_afterRebalancing = 0.0;
	double RiskFreeAsset_afterRebalancing_if = 0.0;
// Initialising for every path with "false". 
	...
	double kosten_aktien_Kauf_Verkauf_if = 0.0;
	double Sum_kosten_aktien_Kauf_Verkauf_if = 0.0;
	double zinskostenAktien_if = 0.0;
	double Sum_zinskostenAktien_if = 0.0;
	double Kosten_Hedge_if = 0.0;
	double delta_old_if= 0.0;
	double gap_Value_if = 0.0; // Das ist differenz zwischen Hedging portfolio vor der Umschichtung und dem aktuellen Derivate Wert 
													// = (delta_old * S_aktuell + riskfreeAsset_vorher) -D_aktuell
	double check_gap_value_after_rebalancing_if = 0.0;
      ...
      //printf("Timestep\n");
		...
		if(tmp_step_noX > N_timeSteps/2. && !DividendsPaid){
		  ...
		  //Check if barrier was touched
			...
			if(tmp_step_noX % 12 == 0){
				printf("in if statemsnt %d\n", tmp_step_noX);
				delta_if = (D_tmp_pls_epsilon - D_tmp )/epsilon;
				gamma1_if = (delta_if-delta_old_if)/(S_tmp_mu - S_tmp_mu_alt_if);
				RiskFreeAsset_beforeRebalancing_if = -(D_tmp_alt_if -(delta_old_if * S_tmp_mu_alt_if))*exp(r_f*delta_t);// Hinzufügen von minus, weil rsik free asset = kredit
				gap_Value_if = (delta_old_if * S_tmp_mu) - RiskFreeAsset_beforeRebalancing_if -D_tmp; // Minus für risk free asset, weil kredit als negativer wert gezählt wurde, Negativer Wert = Höhe der nicht abgesicherten position
				kosten_aktien_Kauf_Verkauf_if = (delta_old_if -delta_if)*S_tmp_mu;// in t_0 delta_old = 0 //  Kosten sind negativ, Erlöse positiv
				Sum_kosten_aktien_Kauf_Verkauf_if += kosten_aktien_Kauf_Verkauf_if;
				Sum_zinskostenAktien_if += zinskostenAktien_if * (exp(r_f*delta_t)-1.); // Zinseszinseffekt auf Zinskonto ??
				//Jetzt erst neue Zinsen aufs konto dazuaddieren. In einem jahr wird konto wieder verzinst...
				zinskostenAktien_if = - delta_t *S_tmp_mu*(exp(r_f*delta_t)-1.); // Kosten sind negativ. Das minus wurde hinzugefuegt, damit Kosten als negative Werte auftreten, wegen des negativen Zinses
				Sum_zinskostenAktien_if += zinskostenAktien_if;
				delta_old_if = delta_if;
				S_tmp_mu_alt_if = S_tmp_mu;
				D_tmp_alt_if = D_tmp;
				RiskFreeAsset_afterRebalancing_if = -(D_tmp - (delta_if * S_tmp_mu));// nagative Werte, wenn 
				check_gap_value_after_rebalancing_if = (delta_if*S_tmp_mu)-RiskFreeAsset_afterRebalancing_if - D_tmp;

	 		 file_output_Delta_if << time << "," << S_tmp_mu << "," << D_tmp << "," << D_tmp_pls_epsilon << "," << D_tmp_min_epsilon << "," << delta_if << "," << gamma << "," 
	 		 << gamma1_if << "," << kosten_aktien_Kauf_Verkauf_if << "," << zinskostenAktien_if << "," << RiskFreeAsset_beforeRebalancing_if << "," 
	 		 << RiskFreeAsset_afterRebalancing_if << "," << gap_Value_if << "," << check_gap_value_after_rebalancing_if << "," << Sum_kosten_aktien_Kauf_Verkauf_if << "," << Sum_zinskostenAktien_if << "\n";
			}
		...
		Kosten_Hedge_if = (Sum_zinskostenAktien_if + Sum_kosten_aktien_Kauf_Verkauf_if - delta_if* S_tmp_mu)*(exp(-r_f*maturity));// discount for one year since maturity is one year
		file_output_Delta_if << "FV_Zinskosten" << "," << "FV_AktienKaufVerkauf" << "," << "FV_delta_S_tmp_muT" << "," << "PV_KostenHedge" << "\n";
		file_output_Delta_if << Sum_zinskostenAktien_if << "," << Sum_kosten_aktien_Kauf_Verkauf_if << "," << delta_if * S_tmp_mu << "," << Kosten_Hedge_if << "\n";
		file_output_Delta_if.close();
