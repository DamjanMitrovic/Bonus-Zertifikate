...
int main(){
	...
	std::vector<int> Diff_Number_Of_Paths     = {50, 100/*, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 1000, 1500, 2000, 2500, 3000*/};
	std::vector<int> Diff_Number_Of_timeSteps = {50, 100/*, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 1000, 1500, 2000, 2500, 3000*/};
  ...
  for(int pathStyle =0; pathStyle <= Diff_Number_Of_Paths.size()-1; pathStyle++){
  	printf("Path = %d\n", Diff_Number_Of_Paths[pathStyle]);
  	for(int timeStep_Style =0; timeStep_Style <= Diff_Number_Of_timeSteps.size()-1; timeStep_Style++){
  		...
  		N_paths 		= Diff_Number_Of_Paths[pathStyle];
			...
			std::string line;
			double tmp_value_file_sigma = 0.0;
			double tmp_SumMeanValue_file = 0.0;
			double tmp_MeanValue_file = 0.0;
			int number_of_prices_mean = 0;
			int number_of_prices_sigma = 0;

			double tmp_sigma = 0.0; // Sum used for calculating sigma: Sum((S_tmp-S_0)²)
			double sigma     = 0.0; // Sigma of the final result calculated as Sqrt(Sum((S_tmp-S_0)²)/N_paths)
			std::size_t a, b;

			while(getline(file, line)){
				tmp_SumMeanValue_file += std::stof(line);
				number_of_prices_mean++;
				}
			file.close();
			tmp_MeanValue_file = tmp_SumMeanValue_file/number_of_prices_mean;
			//printf("Mean Value = %f\n", tmp_MeanValue_file);
			
   		std::ifstream file1("tmp_Prices_Zertifikate" + std::to_string(N_paths) + "_" + std::to_string(N_timeSteps) + ".csv");
			std::string line1;

			while(getline(file1, line1)){
				tmp_value_file_sigma = std::stof(line1);
				tmp_sigma += pow((tmp_value_file_sigma - tmp_MeanValue_file),2);
				number_of_prices_sigma++;
			}
			file1.close();
			//sigma = sqrt(tmp_sigma/(number_of_prices_sigma-1)); // Sigma 
			sigma = sqrt(tmp_sigma/(number_of_prices_sigma*(number_of_prices_sigma-1))); // Standard error

		/*	while(getline(file1, line1)){
				a = line1.find(',', 0);
				b = line1.find('\"', a + 1);
				if (b != std::string::npos){
				std::string tmp_string = line1.substr(a, 10);
				tmp_value_file = std::stof(tmp_string);
				}
				tmp_sigma += pow((tmp_value_file - tmp_MeanValue_file),2);
			}
			file1.close();
			sigma = sqrt(tmp_sigma/(N_paths*(N_paths-1)));
		*/

			file_output << N_paths << "," << N_timeSteps << "," << tmp_MeanValue_file << "," << sigma <<"\n";
